...
g0Result <- ghypMom(0, Theta = gTheta)
g1Result <- ghypMom(1, Theta = gTheta)
g2Result <- ghypMom(2, Theta = gTheta)
detach("package:GeneralizedHyperbolic")
library(HyperbolicDist)
h0Result <- ghypMom(0, Theta = hTheta)
h1Result <- ghypMom(1, Theta = hTheta)
h2Result <- ghypMom(2, Theta = hTheta)
detach("package:HyperbolicDist")
...
if (all(gResult == hResult)) {
  momResult <- "PASS: ghypMom"
} else {
  momResult <- "FAIL: ghypMom"
}
